...
Ese patrón, comprar sistemáticamente en docenas, es exactamente el comportamiento que se espera de un cliente mayorista (un negocio que revende, no una persona comprando para sí misma), consistente con lo ya señalado en la Sección \ref{sec:dataset}: el dataset está dominado por compras de otros negocios, no de consumidores finales. \texttt{secondarysort} permite ver esta clase de patrón por cliente sin tener que volver a leer el dataset completo, la agrupación y el orden secundario ya vienen resueltos por el propio job de MapReduce.
