...
def load_raw():
    ...
    print(f"Engine: {modin_config.Engine.get()}")
    print(f"Particiones objetivo: {modin_config.NPartitions.get()}")
    return pd.read_csv(DATA_PATH, dtype=dtypes)


def query_01_limpieza(df):
    total_antes = len(df)
    limpio = df[df["Description"].notna() & (df["Price"] > 0)]
...
def query_06_top_productos_unidades(df):
    resultado = df.groupby("Description")["Quantity"].sum()
    top10 = resultado.nlargest(10)
    print(top10.to_string())
    return top10
...
def query_07_top_clientes(df):
    ...
    resultado = con_cliente.groupby("Customer ID")["TotalPrice"].sum()
    top10 = resultado.nlargest(10)
    print(top10.to_string())
    return top10
...
def query_08_revenue_por_pais(df):
    ...
    facturas = df.groupby("Country")["Invoice"].nunique()
...
def query_09_revenue_mensual(df):
    mensual = df.groupby(["Year", "Month"])["TotalPrice"].sum()
...
def query_10_tasa_cancelacion_por_pais(df):
    ...
    canceladas_por_pais = df.groupby("Country")["IsCancelled"].sum()
...
def query_11_segmentacion_clientes(df):
    con_cliente = df.dropna(subset=["Customer ID"])
    gasto_por_cliente = con_cliente.groupby("Customer ID")["TotalPrice"].sum()
...
def query_12_ticket_promedio(df):
    ...
    facturas_totales = df["Invoice"].nunique()
    aov_global = revenue_total / facturas_totales
    print(f"AOV global: {aov_global:.2f}")

    revenue_mensual = df.groupby(["Year", "Month"])["TotalPrice"].sum()
    facturas_mensual = df.groupby(["Year", "Month"])["Invoice"].nunique()
